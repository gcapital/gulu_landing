    ...
    def construct(self):
        ...
        def _any(thing, fields=()):
            ...
            if isinstance(thing, QuerySet):
                ...
                if inspect.ismethod(f) and len(inspect.getargspec(f)[0]) == 1:
                    ret = _any(f())            
            #sean            
            elif isinstance(thing, Accessor):
                ret = smart_unicode(thing.url, strings_only=True)            
            #sean            
            else:
        ...
        def _model(data, fields=()):
            ...
            if handler or fields:
                ...
                """Sean: handle the queryset like Manager""" 
                    ...
                    if isinstance(maybe_field, (list, tuple)):
                        ...
                        if inst:
                            if hasattr(inst, 'all'):
                                ...
                                """Sean: handle the image fields"""
                                if model == 'main_profile_pic' or model == 'cover':
                                    ret['photo'] = _model(inst, fields)
                                    #print '========================'
                                else:
                        ...
                        if maybe:
                            if callable(maybe):
                                ...
                                if maybe_field == 'image600x400':
                                    ret['image_large'] = _any(maybe)
                                elif maybe_field == 'image185x185':
                                    ret['image_medium'] = _any(maybe)
                                elif maybe_field == 'image50x50':
                                    ret['image_small'] = _any(maybe)
                                else:
                                    ret[maybe_field] = _any(maybe)                                
                        else:
